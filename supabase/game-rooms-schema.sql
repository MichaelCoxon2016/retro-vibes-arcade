...
CREATE TABLE IF NOT EXISTS public.game_rooms (
  ...
  game_type VARCHAR(50) NOT NULL DEFAULT 'snake', -- snake, parkour, etc.
  game_mode VARCHAR(20) NOT NULL DEFAULT 'pvp', -- pvp, tournament
  status VARCHAR(20) NOT NULL DEFAULT 'waiting', -- waiting, ready, playing, finished, cancelled
  settings JSONB DEFAULT '{}', -- Game-specific settings (difficulty, speed, etc.)
  ...
  -- Add check constraint to ensure proper status transitions
...
CREATE TABLE IF NOT EXISTS public.game_states (
  ...
  state JSONB NOT NULL, -- Player state (position, direction, score, etc.)
  sequence_number INTEGER NOT NULL DEFAULT 0, -- For ordering updates
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  
  -- Ensure unique updates per player per sequence
...
CREATE TABLE IF NOT EXISTS public.game_events (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  room_id UUID NOT NULL REFERENCES public.game_rooms(id) ON DELETE CASCADE,
  event_type VARCHAR(50) NOT NULL, -- food_eaten, powerup_collected, player_died, game_over
...
-- Game rooms policies
CREATE POLICY "Anyone can view active game rooms" ON public.game_rooms
  FOR SELECT USING (status IN ('waiting', 'ready', 'playing'));

CREATE POLICY "Authenticated users can create game rooms" ON public.game_rooms
  FOR INSERT WITH CHECK (auth.uid() = host_id);

CREATE POLICY "Room participants can update game rooms" ON public.game_rooms
  FOR UPDATE USING (auth.uid() IN (host_id, guest_id));

CREATE POLICY "Host can delete their game rooms" ON public.game_rooms
  FOR DELETE USING (auth.uid() = host_id);

-- Game states policies
CREATE POLICY "Room participants can view game states" ON public.game_states
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Room participants can insert game states" ON public.game_states
  FOR INSERT WITH CHECK (
    auth.uid() = game_states.player_id AND
    EXISTS (
      SELECT 1 FROM public.game_rooms
      WHERE game_rooms.id = game_states.room_id
      AND auth.uid() IN (game_rooms.host_id, game_rooms.guest_id)
...
CREATE POLICY "Room participants can view game events" ON public.game_events
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.game_rooms
...
CREATE POLICY "Room participants can create game events" ON public.game_events
  FOR INSERT WITH CHECK (
    auth.uid() = game_events.created_by AND
    EXISTS (
      SELECT 1 FROM public.game_rooms
      WHERE game_rooms.id = game_events.room_id
      AND auth.uid() IN (game_rooms.host_id, game_rooms.guest_id)
